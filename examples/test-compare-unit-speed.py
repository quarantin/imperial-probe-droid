...
async def __main__():
	...
	if os.path.exists('custom-players.json'):
		...
		players = await api_swgoh_players(config, project={ 'allycodes': full_ally_codes })
	...
	if os.path.exists('custom-stats.json'):
		...
		stats, players = await fetch_crinolo_stats(config, full_ally_codes, players=players)
