...
def get_full_avatar_url(config, image, unit):
	...
	return '%s/avatar/%s?level=%s&gear=%s&rarity=%s&zetas=%s&relics=%s&alignment=%s&version=1' % (config.get_server_url(), base_id, level, gear, rarity, zetas, relics, alignment)
